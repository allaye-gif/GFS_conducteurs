...
- **Humidité** (`SCALES.humidity`, HU850/HU700 partagent la même config) : **classification en 4 catégories nettes**, pas un dégradé (ni fin ni à bandes 10%) — sec (<30%, invisible/blanc), modéré (30-60%, rose clair), humide (60-85%, magenta), saturé (>85%, magenta franc `#e6199a`). Couleurs choisies à la main (`RenderOptions.bandColors`, pas dérivées d'un dégradé via `deriveBands`) pour un contraste net entre classes plutôt qu'une transition. **Isolignes désactivées** (`ContourOptions.hideLines: true`) — c'est le remplissage catégoriel qui porte l'info, pas des traits à suivre. Titre posé directement sans cartouche (`plainTitle`). Fond blanc.
  - **Historique (3 itérations rejetées avant celle-ci)** : (1) isohumes seules sur fond parchemin façon Synergie brute — pas de repère visuel hors des lignes ; (2) remplissage magenta isolé uniquement au-delà de 95%, reste en isolignes sur fond quasi-vide — pas de repère sur le reste de la carte ; (3) bandes fines tous les 10% (9 paliers, même mécanisme que Temp) — encore jugé "pas assez compréhensible", trop proche d'un dégradé continu pour distinguer sec/humide/saturé d'un coup d'œil. La bonne réponse était moins de catégories (4) avec des couleurs beaucoup plus tranchées entre elles, pas plus de paliers.
  - **Leçon** : "bandes discrètes" ne suffit pas en soi — il faut aussi que le NOMBRE de classes soit petit et que les couleurs entre classes soient nettement différentes (pas dérivées d'un dégradé lissé), sinon ça se voit et se lit comme un dégradé continu.
- **Vent** (`SCALES.wind`/`windUpper`) : aplat dégradé continu classique (axes, légende barre+graduations, villes) — style dashboard d'origine, inchangé (explicitement validé ainsi). FF10/FF850 ont en plus des barbules météo (`windBarbSvg`) par-dessus le dégradé (échantillonnage plus lâche). FF850 utilise `windUpper` (0-25 m/s, flux d'altitude plus fort).
- **Précip** (`SCALES.precip`, RR3H/RR6H) : style "isohyètes Synergie" — contrairement à temp/humidité qui varient en douceur sur de grandes zones, la précipitation est spatialement très localisée (cellules orageuses isolées), donc pas de sens à un aplat régional lisse. `withFill` + `bandBoundaries: [1,5,10,20,30]` + `bandColors` choisis à la main (pas dérivés d'un dégradé) : <1mm invisible sur fond parchemin, puis bleu très pâle → cyan → vert → bleu → violet (cœur des cellules les plus intenses). Isolignes bleues gardées par-dessus (contrairement à l'humidité qui les a retirées) — servent à dessiner le contour exact de chaque cellule de pluie, pas juste son intensité. Fond parchemin (`background: "cream"`, comme la capture Synergie).
