...
export function traceStreamlines(
  ...
  // qu'un fouillis.
  const occCols = Math.max(1, Math.round(lonSpan / seedSpacingDeg)) + 1;
  const occRows = Math.max(1, Math.round(latSpan / seedSpacingDeg)) + 1;
  const occupied = new Uint8Array(occCols * occRows);
  const occIndex = (lon: number, lat: number) => {
    const c = Math.min(occCols - 1, Math.max(0, Math.round(((lon - lon0) / lonSpan) * (occCols - 1))));
    const r = Math.min(occRows - 1, Math.max(0, Math.round(((lat - lat0) / latSpan) * (occRows - 1))));
    return r * occCols + c;
  };
  ...
  for (let row = 0; row < occRows; row++) {
    const lat = lat0 + (row / (occRows - 1 || 1)) * latSpan;
    for (let col = 0; col < occCols; col++) {
      const lon = lon0 + (col / (occCols - 1 || 1)) * lonSpan;
